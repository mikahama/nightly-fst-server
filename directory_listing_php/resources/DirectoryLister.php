...
class DirectoryLister {
    ...
    public function getMetadata(){
        $string = file_get_contents("metadata.json");
        if ($string === false) {
            return "<p>Metadata is missing!</p>";
        }

        $arr = json_decode($string, true);
        if ($arr === null) {
            return "<p>Metadata is malformed</p>";
        }
        $html = "";
        if ($arr && is_array($arr)) {
...
private static function _arrayToHtmlTableRecursive($arr) {
    ...
    foreach ($arr as $key => $val) {
        ...
        if (is_array($val)) {
            ...
            $val = preg_replace($url, '<a href="$0" target="_blank" >$0</a>', $val);
            $str .= "<i>$val</i>";
        }
        $str .= "</td></tr>";
    }
    ...
    public function getLang(){
        ...
        $parts = explode("/", $url);
        $isoCode =  end($parts);
        return Locale::getDisplayLanguage($isoCode, 'en');
    }
